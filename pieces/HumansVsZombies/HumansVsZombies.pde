...
class Human extends Vehicle {
  ...
  PVector wander(){
    ...
    return PVector.add(PVector.add(copy(position), PVector.mult(PVector.normalize(copy(velocity)), wanderDistance)), PVector.add(wanderRadius*cos(rotation+velocity.heading()), wanderRadius*sin(rotation+velocity.heading())));
  }
}
...
abstract class Vehicle {
  ...
  Vehicle update() {
    forward = PVector.normalize(copy(velocity));                               //set the forward to the normal of the velocity
  ...
  void findClosest(ArrayList subjects){
    ...
    for(Vehicle v : (ArrayList<Vehicle>)subjects){
      ...
      if((tempDist = dist(v.position.x, v.position.y, position.x, position.y)) < minDist){
        ...
        target = PVector.add(copy(v.position), PVector.mult(v.velocity, 7));
  ...
  */
  ...
  PVector seek(PVector target) {
    desired = PVector.mult(PVector.normalize(PVector.sub(target, position)), maxSpeed);
